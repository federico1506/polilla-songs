...
function App() {
  ...
  const formatLyrics = (lyrics: string[]) => {
    return lyrics.map((line, index) => {
      ...
      const sectionWords = [
        'ESTROFA', 'ESTRIBILLO', 'SOLO', 'PUENTE', 'RAP', 'MODULA', 'PRE-ESTRIBILLO', 'NANANA'
      ];
      const upperLine = line.trim().toUpperCase();
      const isSection = sectionWords.some(word =>
        upperLine === word ||
        /^([A-ZÁÉÍÓÚÜÑ]+) [0-9]+$/.test(upperLine) && upperLine.startsWith(word + ' ') ||
  ...
  return (
    <div className="App">
      {/* Header con imagen del nombre */}
      <header className="header">
        <div className="container header-content" style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', padding: '2.5rem 0 1.5rem 0' }}>
          <img src={nombrePolilla} alt="Polilla Banda Bahiense" className="nombre-polilla-img" />
        </div>
      </header>
        ...
        <div className="container">
          ...
          <div className="songs-grid">
            {allSongs.map((song, index) => (
              ...
              >
                <img src={logoPolilla} alt="Logo Polilla" className="song-card-logo" style={{ width: '38px', height: '38px', position: 'absolute', top: '18px', right: '18px', opacity: 0.7, pointerEvents: 'none' }} />
        ...
        <div className="container members-list-uno">
          ...
          <div className="members-grid-uno">
            {bandMembers.map((member, index) => (
              <div key={member.name} className="member-card-uno fade-in-up" style={{ animationDelay: `${index * 0.1}s` }}>
                <div className="member-info-uno">
                  <div className="member-icon-uno">{instrumentIcons[member.instrument]}</div>
                  <div>
                ...
                <img src={logoPolilla} alt="Logo Polilla" className="member-logo-uno" />
              </div>
            ))}
          </div>
        </div>
